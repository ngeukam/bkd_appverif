...
// Agrégation pour calculer ce qui revient à la plateforme
const calculatePlatformRevenue = async (req, res) => {
  try {
    const revenue = await Project.aggregate([
      // 1. Joindre avec AcceptedProject pour récupérer les tests complétés
      {
        $lookup: {
          from: "acceptedprojects", // Collection AcceptedProject
          localField: "_id", // Liaison via le champ project
          foreignField: "project",
          as: "acceptedProjects",
        },
      },
      // 2. Filtrer les projets où le status_of_test est "completed"
      {
        $match: {
          "acceptedProjects.status_of_test": "completed",
        },
      },
      // 3. Joindre avec Payment pour vérifier les paiements complétés
      {
        $lookup: {
          from: "payments", // Collection Payment
          localField: "_id", // Liaison via le champ project
          foreignField: "project",
          as: "payments",
        },
      },
      // 4. Filtrer les paiements où le statut est "completed"
      {
        $match: {
          "payments.status": "completed",
        },
      },
      // 5. Calculer le revenu de la plateforme pour chaque projet
      {
        $project: {
          _id: 1,
          name: 1,
          amount: 1,
          commission: 1,
          platformRevenue: { $multiply: ["$commission", "$amount"] }, // Revenu pour chaque projet
        },
      },
      // 6. Agréger le revenu total
      {
        $group: {
          _id: null,
          totalPlatformRevenue: { $sum: "$platformRevenue" }, // Somme des revenus
        },
      },
    ]);
    return res.status(200).json({
      error: false,
      result: revenue.length > 0 ? revenue[0].totalPlatformRevenue : 0,
    });
  } catch (error) {
    console.error(error);
    return res.status(500).json({
      error: true,
      msg: "Failed to evaluate total platform revenue",
    });
  }
};
